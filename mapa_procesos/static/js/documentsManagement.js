document.addEventListener('DOMContentLoaded', function () {
    ...
    var btnInsertar = document.getElementById('insertar_documentos')
    ...
    btnInsertar.onclick = function () {
        const modelName = document.getElementById('modeloBase').innerText // Obtener el nombre del modelo
    ...
    document.addEventListener('click', function (event) {
        if (event.target.matches('[id^="eliminar_documentos_"]')) {
            event.preventDefault()
            const documentId = event.target.id.split('_')[2]
            const fileName = event.target.getAttribute('data-nombre')
            const modelName = document.getElementById('modeloBase').innerText
    ...
    function cargarDocumentos() {
        const modelName = document.getElementById('modeloBase').innerText // Obtener el nombre del modelo
        console.log('Cargando documentos para:', modelName)
        fetch(`/buscar-documentos-especificos/${modelName}/`)
            .then((response) => {
    ...
    function asignarUrls() {
        // Obtén el valor de localStorage
        const modeloBase = localStorage.getItem('modeloBase');
        const image_form = localStorage.getItem('img');
        const formatos_form = localStorage.getItem('modelo');

        document.querySelectorAll('a[id^="mi-enlace-documentos_"]').forEach((link) => {
            const documentId = link.getAttribute('data-id');
            if (documentId && modeloBase) {
                const url = `/ver-pdf/${documentId}/${modeloBase}/${image_form}/${formatos_form}/`; // Construye la URL
                link.href = url; // Asigna la URL al enlace
            }
        });
    }
    cargarDocumentos()
    document.getElementById('insertar_documentos').addEventListener('click', function () {
